// adapters for use with lantern.  This mostly is in place to allow the lantern backend to handle
// upstream connections itself and behave like some other transports.
package lantern
...
func maybeLocalDialer(isLocal HandleLocalPredicate, handleLocal service.TargetDialer, handleUpstream service.TargetDialer) service.TargetDialer {
	return func(tgtAddr string, clientAddr net.Addr, proxyMetrics *metrics.ProxyMetrics, targetIPValidator onet.TargetIPValidator) (onet.DuplexConn, *onet.ConnectionError) {
		if isLocal(tgtAddr) {
...
func (l *llistener) dialPipe(addr string, clientAddr net.Addr, proxyMetrics *metrics.ProxyMetrics, targetIPValidator onet.TargetIPValidator) (onet.DuplexConn, *onet.ConnectionError) {
	c1, c2 := net.Pipe()
