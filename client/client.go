...
type Client interface {
	...
	DialTCP(laddr *net.TCPAddr, raddr string, opts ...*DialTCPOptions) (onet.DuplexConn, error)
...
func (c *ssClient) DialTCP(laddr *net.TCPAddr, raddr string, opts ...*DialTCPOptions) (onet.DuplexConn, error) {
	var prefixBuf []byte = nil
	if opts != nil {
		if len(opts) > 1 {
			return nil, fmt.Errorf("at most one DialTCPOption argument is allowed")
		}
		if opts[0] != nil && opts[0].PrefixBuf != nil {
			prefixBuf = opts[0].PrefixBuf
		}
	}
	...
	// fmt.Printf("Shadowsocks: Connected to proxy %s\n", proxyConn.RemoteAddr())
	ssw := ss.NewShadowsocksWriter(proxyConn, c.cipher, prefixBuf)
	if c.salter != nil {
